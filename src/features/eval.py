...
class EvaluadorSenal:
    ...
    def evaluar_flags(self, flags_list):
        ...
        for item in flags_list:
            ...
            etiqueta = item[3] if len(item) > 3 else ("global (nulo=0)" if pop is None else "con historial")
            ...
            # la última medición de cada feature manda: listar siempre global primero y condicionada después
            self._registrar(self.fb_flags, {"feature": name, "tipo": "flag", "poblacion": etiqueta,
                                            "efecto": (p1 - p0) * 100, "p": pv, "n_pos": n1})

        from IPython.display import display  # import perezoso: no acopla src.features a IPython
        display(pd.DataFrame(rows).style.hide(axis='index'))

    def evaluar_continuas(self, cont_list):
        ...
        for item in cont_list:
            ...
            self.fb_comparisons.append((name, etiqueta))
            self._registrar(self.fb_cont, {"feature": name, "tipo": "continua", "poblacion": etiqueta,
                                           "efecto": r, "p": pv, "n_pos": len(vals)})
